...
var RLANG = {
  ...
  backcolor: 'Цвет фона',
  unorderedlist: 'Ненумерованный списон',
  orderedlist: 'Нумерованный списон',
  ...
  mailto: 'Мыло',
  web: 'Ссыль',
  video_html_code: 'Код вставки видео',
  file: 'Вставить файл',
  upload: 'Загрузить',
  download: 'Скачать',
  choose: 'Выбрать',
  or_choose: 'Или выбрать',
  drop_file_here: 'Перетащить файл сюда',
  align_left:  'Выровнять по левому краю',
  align_center: 'Выровнять по центру',
  align_right: 'Выровнять по правому краю',
  align_justify: 'Растянуть по ширине',
  horizontalrule: 'Вставить линию отбивки',
  deleted: 'Удалено',
  anchor: 'Якорь',
  link_new_tab: 'Открыть в новом окне',
...
(function($){
  ...
  var Redactor = function(element, options){
    ...
    this.opts = $.extend({
      ...
      wym: false,
      convertLinks: false,
      convertDivs: false, // convert div to paragraphs
      ...
      airButtons: ['html', 'formatting', '|', 'bold', 'italic', 'deleted', '|', 'unorderedlist', 'orderedlist', 'outdent', 'indent', '|', 'fontcolor', 'backcolor'],
      ...
      modal_file: String() +
        ...
        '<form id="redactorUploadFileForm" method="post" action="" enctype="multipart/form-data">' +
          '<label>Имя (не обязательно)</label>' +
  ...
  Redactor.prototype = {
    ...
    keydown: function(){
      this.$editor.keydown($.proxy(function(e){
        ...
        // ---------------------------------------------
        // ENTER - просто добавляем <br /> 
        // (если убрать этот блок - будет вставлять div)
        // ---------------------------------------------
        if (pre === false && key === 13){
          e.preventDefault();
          //this.insertNodeAtCaret($('<br>').get(0));
          this.insertHtml("<br />");
          //this.syncCode();
          //return false;
        }
