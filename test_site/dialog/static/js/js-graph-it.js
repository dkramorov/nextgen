...
// --------------------------
// Поднимаемся пока не найдем
// специфический class
// --------------------------
function findAncestor (el, cls) {
  if (hasClass(el, cls)) return el;
  // -------------------------------
  // Если где-то работать не будет
  // classList, то можно заменить на
...
function startDrag(e){
	...
	// --------------------------------------
	// Если кликнули по документу или
	// редактору - НЕ начинаем перетаскивание
	// --------------------------------------
	who_is_your_daddy = findAncestor(eventSource, 'redactor_editor');

	if(eventSource.tagName == 'HTML' || hasClass(who_is_your_daddy, 'redactor_editor'))
		return;
...
function HorizontalSStrategy(connector){
	...
	this.paint = function(){
		...
		// ----------------------------
		// Ровная горизонтальная линия,
		// можно скрывать при желении
		// ----------------------------
		//if(vLength == 0)
